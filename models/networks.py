...
from models.layers import unetConv3
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='instance',
    ...
    if netG == 'resnet_9blocks':
        ...
        net =ResnetGenerator(3, 3, 64, norm_layer="instance", use_dropout=True,
                        no_antialias='store_true', no_antialias_up='store_true', n_blocks=9, opt=opt)
        ...
        net = ResnetGenerator(3, 3, 64, norm_layer="instance", use_dropout=True,
                              no_antialias='store_true', no_antialias_up='store_true', n_blocks=6, opt=opt)
        ...
        #
        # net = UnetGenerator(3, 3, 7, 64, norm_layer=nn.BatchNorm2d, use_dropout=True)
    ...
    elif netG == 'UNet_3Plus_DeepSup':
        # net = UNet_3Plus_Mamba_DeepSup(in_channels=3, n_classes=3, feature_scale=4)

        net = UNet_3Plus_Mamba_DeepSup(in_channels=3, n_classes=3)
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9,
        ...
        if isinstance(norm_layer, str):  # 防止 norm_layer 传入字符串
...
# --------------------------------------------------------------------------
try:
    from mamba_ssm import Mamba as MambaOrigin
    HAS_MAMBA = True
except ImportError:
    HAS_MAMBA = False
    print("Warning: mamba_ssm not found. Using PyTorch fallback.")
...
class MambaLayer(nn.Module):
    def __init__(self, d_model, d_state=16, d_conv=4, expand=2):
        super().__init__()
        if HAS_MAMBA:
            self.mamba = MambaOrigin(d_model=d_model, d_state=d_state, d_conv=d_conv, expand=expand)
        else:
            self.in_proj = nn.Linear(d_model, d_model * expand * 2)
            self.conv1d = nn.Conv1d(in_channels=d_model * expand, out_channels=d_model * expand,
                                    bias=True, kernel_size=d_conv, groups=d_model * expand, padding=d_conv - 1)
            self.x_proj = nn.Linear(d_model * expand, (d_state + d_state * 2), bias=False)
            self.dt_proj = nn.Linear(d_state, d_model * expand, bias=True)
...
class Downsample_Subpixel(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(Downsample_Subpixel, self).__init__()
...
class VSSM_Block(nn.Module):
    def __init__(self, in_channels, d_state=16):
        ...
        self.silu = nn.SiLU()
        self.ssm = MambaLayer(d_model=in_channels, d_state=d_state)
        self.norm = nn.LayerNorm(in_channels)
        self.linear_gate = nn.Conv2d(in_channels, in_channels, 1)
...
class MambaFusionNode(nn.Module):
    def __init__(self, in_channels, out_channels, d_state=16):
        super(MambaFusionNode, self).__init__()
        self.project = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 1, bias=False),
            nn.InstanceNorm2d(out_channels, affine=True),
            nn.ReLU(inplace=True)
...
class GatedSkipConnection(nn.Module):
    """
    Skip connection with learnable weights and Dropout to prevent low-level features 
    from interfering with deep semantic translation.
    """
    def __init__(self, in_channels, out_channels, dropout_prob=0.1):
        super(GatedSkipConnection, self).__init__()
        self.gate = nn.Parameter(torch.ones(1, out_channels, 1, 1) * 0.1) # Initially assign a small weight
    ...
    def forward(self, x):
        return self.dropout(self.project(x) * self.gate)
...
class UNet_3Plus_Mamba_DeepSup(nn.Module):
    def __init__(self, in_channels=3, n_classes=3, deep_supervision=True, dropout_skip=0.1):
        ...
        self.up_channels = self.cat_channels * 5

        # 1. Intro
        self.intro_unshuffle = nn.PixelUnshuffle(downscale_factor=2)
        self.intro_conv = nn.Sequential(
            nn.Conv2d(in_channels * 4, self.filters[0], 3, padding=1),
            nn.InstanceNorm2d(self.filters[0], affine=True),
            nn.ReLU(True)
        )

        # 2. Encoder
        self.X_En_1 = ConvBlock(self.filters[0], self.filters[0])
        self.down_1_2 = Downsample_Subpixel(self.filters[0], self.filters[0])
        self.X_En_2 = ConvBlock(self.filters[0], self.filters[1])
        self.down_2_3 = Downsample_Subpixel(self.filters[1], self.filters[1])
        self.X_En_3 = ConvBlock(self.filters[1], self.filters[2])
        self.down_3_4 = Downsample_Subpixel(self.filters[2], self.filters[2])
        self.X_En_4 = ConvBlock(self.filters[2], self.filters[3])
        self.down_4_5 = Downsample_Subpixel(self.filters[3], self.filters[3])
        self.X_En_5 = ConvBlock(self.filters[3], self.filters[4])

        # ----------------------------------------------------------------------
        # Initialize gated skip connection components
        # ----------------------------------------------------------------------
        # Gating for Nested Nodes
        self.gate_En1_to_01 = GatedSkipConnection(self.filters[0], self.filters[0], dropout_skip)
        self.gate_En2_to_11 = GatedSkipConnection(self.filters[1], self.filters[1], dropout_skip)
        self.gate_En3_to_21 = GatedSkipConnection(self.filters[2], self.filters[2], dropout_skip)

        # Gating for De4
        self.gate_En1_to_De4 = GatedSkipConnection(self.cat_channels, self.cat_channels, dropout_skip)
        self.gate_En2_to_De4 = GatedSkipConnection(self.cat_channels, self.cat_channels, dropout_skip)
        self.gate_En3_to_De4 = GatedSkipConnection(self.cat_channels, self.cat_channels, dropout_skip)

        # Gating for De3
        self.gate_En1_to_De3 = GatedSkipConnection(self.cat_channels, self.cat_channels, dropout_skip)
        self.gate_En2_to_De3 = GatedSkipConnection(self.cat_channels, self.cat_channels, dropout_skip)

        # Gating for De2
        self.gate_En1_to_De2 = GatedSkipConnection(self.cat_channels, self.cat_channels, dropout_skip)

        # 3. Nested Nodes
        self.X_0_1 = ConvBlock(self.filters[0] + self.filters[1], self.filters[0])
        self.X_0_2 = ConvBlock(self.filters[0] * 2 + self.filters[1], self.filters[0])
        self.X_0_3 = ConvBlock(self.filters[0] * 3 + self.filters[1], self.filters[0])
        self.X_1_1 = ConvBlock(self.filters[1] + self.filters[2], self.filters[1])
        self.X_1_2 = ConvBlock(self.filters[1] * 2 + self.filters[2], self.filters[1])
        self.X_2_1 = ConvBlock(self.filters[2] + self.filters[3], self.filters[2])

        # 4. Decoder Nodes
        self.conv_En1_to_De4 = nn.Sequential(Downsample_Subpixel(self.filters[0], self.filters[0]),
                                             Downsample_Subpixel(self.filters[0], self.filters[0]),
                                             Downsample_Subpixel(self.filters[0], self.filters[0]),
                                             SingleConv(self.filters[0], self.cat_channels))
        self.conv_En2_to_De4 = nn.Sequential(Downsample_Subpixel(self.filters[1], self.filters[1]),
                                             Downsample_Subpixel(self.filters[1], self.filters[1]),
                                             SingleConv(self.filters[1], self.cat_channels))
        self.conv_En3_to_De4 = nn.Sequential(Downsample_Subpixel(self.filters[2], self.filters[2]),
                                             SingleConv(self.filters[2], self.cat_channels))
        self.conv_En4_to_De4 = SingleConv(self.filters[3], self.cat_channels)
        self.conv_En5_to_De4 = SingleConv(self.filters[4], self.cat_channels)
        self.X_De_4_Fusion = MambaFusionNode(self.up_channels, self.filters[3], d_state=16)

        self.conv_En1_to_De3 = nn.Sequential(Downsample_Subpixel(self.filters[0], self.filters[0]),
                                             Downsample_Subpixel(self.filters[0], self.filters[0]),
                                             SingleConv(self.filters[0], self.cat_channels))
        self.conv_En2_to_De3 = nn.Sequential(Downsample_Subpixel(self.filters[1], self.filters[1]),
                                             SingleConv(self.filters[1], self.cat_channels))
        self.conv_X21_to_De3 = SingleConv(self.filters[2], self.cat_channels)
        self.conv_De4_to_De3 = SingleConv(self.filters[3], self.cat_channels)
        self.conv_En5_to_De3 = SingleConv(self.filters[4], self.cat_channels)
        self.X_De_3_Fusion = MambaFusionNode(self.up_channels, self.filters[2], d_state=16)

        self.conv_En1_to_De2 = nn.Sequential(Downsample_Subpixel(self.filters[0], self.filters[0]),
                                             SingleConv(self.filters[0], self.cat_channels))
        self.conv_X12_to_De2 = SingleConv(self.filters[1], self.cat_channels)
        self.conv_De3_to_De2 = SingleConv(self.filters[2], self.cat_channels)
        self.conv_De4_to_De2 = SingleConv(self.filters[3], self.cat_channels)
        self.conv_En5_to_De2 = SingleConv(self.filters[4], self.cat_channels)
        self.X_De_2_Fusion = MambaFusionNode(self.up_channels, self.filters[1], d_state=16)

        self.conv_X03_to_De1 = SingleConv(self.filters[0], self.cat_channels)
        self.conv_De2_to_De1 = SingleConv(self.filters[1], self.cat_channels)
        self.conv_De3_to_De1 = SingleConv(self.filters[2], self.cat_channels)
        self.conv_De4_to_De1 = SingleConv(self.filters[3], self.cat_channels)
        self.conv_En5_to_De1 = SingleConv(self.filters[4], self.cat_channels)
        self.X_De_1_Fusion = MambaFusionNode(self.up_channels, self.filters[0], d_state=16)

        # 5. Outro
        self.outro_up = nn.Conv2d(self.filters[0], self.filters[0] * 4, 3, padding=1)
        self.outro_shuffle = nn.PixelShuffle(upscale_factor=2)
        self.head_identity = nn.Conv2d(in_channels, self.filters[0], 3, padding=1)

        self.global_refine = nn.Sequential(ResidualBlock(self.filters[0]), ResidualBlock(self.filters[0]),
                                           ResidualBlock(self.filters[0]))

        if self.deep_supervision:
            self.final1 = nn.Conv2d(self.filters[0], n_classes, 1)
            self.final2 = nn.Conv2d(self.filters[1], n_classes, 1)
            self.final3 = nn.Conv2d(self.filters[2], n_classes, 1)
            self.final4 = nn.Conv2d(self.filters[3], n_classes, 1)
            self.final_main = nn.Conv2d(self.filters[0], n_classes, 1)
        else:
            self.final_main = nn.Conv2d(self.filters[0], n_classes, 1)
    ...
    def _up_bicubic(self, x, scale_factor):
        return F.interpolate(x, scale_factor=scale_factor, mode='bicubic', align_corners=True)

    def forward(self, inputs, layers=[], encode_only=False):
        # 1. Intro
        x_intro = self.intro_conv(self.intro_unshuffle(inputs))

        # 2. Encoder
        X_En_1 = self.X_En_1(x_intro)
        X_En_2 = self.X_En_2(self.down_1_2(X_En_1))
        X_En_3 = self.X_En_3(self.down_2_3(X_En_2))
        X_En_4 = self.X_En_4(self.down_3_4(X_En_3))
        ...
        if encode_only:
            ...
            if len(layers) > 0:
                ...
                for layer_id in layers:
                    if layer_id < len(all_feats):
                        return_feats.append(all_feats[layer_id])
                return return_feats
            else:
                return all_feats
        ...
        X_0_2 = self.X_0_2(torch.cat([X_En_1, X_0_1, self._up_bicubic(X_1_1, 2)], 1))
        ...
        h3 = self.gate_En3_to_De4(self.conv_En3_to_De4(X_En_3))
        h4 = self.conv_En4_to_De4(X_En_4)
        h5 = self.conv_En5_to_De4(self._up_bicubic(X_En_5, 2))
        X_De_4 = self.X_De_4_Fusion(torch.cat([h1, h2, h3, h4, h5], 1))

        # X_De_3
        h1 = self.gate_En1_to_De3(self.conv_En1_to_De3(X_En_1))
        h2 = self.gate_En2_to_De3(self.conv_En2_to_De3(X_En_2))
        h3 = self.conv_X21_to_De3(X_2_1)
        h4 = self.conv_De4_to_De3(self._up_bicubic(X_De_4, 2))
        h5 = self.conv_En5_to_De3(self._up_bicubic(X_En_5, 4))
        X_De_3 = self.X_De_3_Fusion(torch.cat([h1, h2, h3, h4, h5], 1))

        # X_De_2
        h1 = self.gate_En1_to_De2(self.conv_En1_to_De2(X_En_1))
        h2 = self.conv_X12_to_De2(X_1_2)
        h3 = self.conv_De3_to_De2(self._up_bicubic(X_De_3, 2))
        h4 = self.conv_De4_to_De2(self._up_bicubic(X_De_4, 4))
        h5 = self.conv_En5_to_De2(self._up_bicubic(X_En_5, 8))
        X_De_2 = self.X_De_2_Fusion(torch.cat([h1, h2, h3, h4, h5], 1))

        # X_De_1
        h1 = self.conv_X03_to_De1(X_0_3)
        h2 = self.conv_De2_to_De1(self._up_bicubic(X_De_2, 2))
        h3 = self.conv_De3_to_De1(self._up_bicubic(X_De_3, 4))
        h4 = self.conv_De4_to_De1(self._up_bicubic(X_De_4, 8))
        h5 = self.conv_En5_to_De1(self._up_bicubic(X_En_5, 16))
        X_De_1 = self.X_De_1_Fusion(torch.cat([h1, h2, h3, h4, h5], 1))
        ...
        if self.deep_supervision:
            out1 = self._up_bicubic(self.final1(X_De_1), 2)
            out2 = self._up_bicubic(self.final2(X_De_2), 4)
            out3 = self._up_bicubic(self.final3(X_De_3), 8)
            out4 = self._up_bicubic(self.final4(X_De_4), 16)
            out_main = self.final_main(x_final)
            return [torch.tanh(out1), torch.tanh(out2), torch.tanh(out3), torch.tanh(out4), torch.tanh(out_main)]
        else:
            return torch.tanh(self.final_main(x_final))
