...
class CUTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--nce_includes_all_negatives_from_minibatch',
                            ...
                            help='(used for single image translation) If True, include the negatives from the other samples of the minibatch.')
        ...
        if opt.CUT_mode.lower() == "cut":
            ...
            parser.set_defaults(nce_idt=False, lambda_NCE=10.0, flip_equivariance=True,
    ...
    def forward(self):
        ...
        # 3. [Critical fix] Handle the issue of Deep Supervision returning a list
        # If it returns a List, take the last one (Main Output)
        if isinstance(self.fake, list):
            self.fake_final = self.fake[-1]
        else:
            self.fake_final = self.fake

        # 4. Split Batch
        self.fake_B = self.fake_final[:self.real_A.size(0)]
        if self.opt.nce_idt:
            self.idt_B = self.fake_final[self.real_A.size(0):]
    ...
    def calculate_NCE_loss(self, src, tgt):
        ...
        # 1. Query features (Fake) - pass encode_only=True
        feat_q = self.netG(tgt, self.nce_layers, encode_only=True)

        if self.opt.flip_equivariance and self.flipped_for_equivariance:
            feat_q = [torch.flip(fq, [3]) for fq in feat_q]

        # 2. Key features (Real) - pass encode_only=True
