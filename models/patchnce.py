# ///////////////////////////////////////////////////////////////////////////////   L-sfotmax+ArcFace + Hard Negative Mining
...
class PatchNCELoss(nn.Module):
    def __init__(self, opt):
        ...
        self.margin = getattr(opt, 'lsoftmax_margin', 0.2) 
        self.eps = 1e-6
    ...
    def forward(self, feat_q, feat_k):
        ...
        # -----------------------------------------------------------
        # -----------------------------------------------------------
        cos_theta = torch.sum(feat_q * feat_k, dim=1, keepdim=True)
        ...
        if hasattr(self.opt, 'nce_includes_all_negatives_from_minibatch') and self.opt.nce_includes_all_negatives_from_minibatch:
            batch_dim_for_bmm = 1
        else:
            batch_dim_for_bmm = self.opt.batch_size

        # reshape features
        feat_q_view = feat_q.view(batch_dim_for_bmm, -1, dim)
        feat_k_view = feat_k.view(batch_dim_for_bmm, -1, dim)
        npatches = feat_q_view.size(1)
        ...
        # -----------------------------------------------------------
        # 5. Concat & Cross Entropy
        # -----------------------------------------------------------
        out = torch.cat((l_pos, l_neg_hard), dim=1) / self.opt.nce_T
